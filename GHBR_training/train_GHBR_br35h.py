...
def calculate_mean_std(dataset, img_size):
    ...
    mean = feats_sum / length  # mean
    std = std_sum / length  # Std deviation
...
def main_worker(gpu, args):
    ...
    # Save granular rectangle data          
    output_dir = r'E:/OURS-update/br35h_h5'
    os.makedirs(output_dir,exist_ok=True)
    process_data(train_dset,output_dir,'train')
    process_data(eval_dset,output_dir,'val')
    print('h5data finish')
    
    #Generate granular rectangle dataset 
    from Data_loader.ClassificationBase_dataset import Br35HDataset
    GRdataset = Br35HDataset(mode='train',args=args)  # train set
    GRval_dataset = Br35HDataset(mode='val',args=args)

     
    #Calculate mean and std of the granular rectangle dataset
    mean1,std1 = calculate_mean_std(GRdataset,64)  # args.img_size=64
    mean2,std2 = calculate_mean_std(GRval_dataset,64)
    print("train data number:{},eval data number:{}".format(len(GRdataset), len(GRval_dataset)))
    print("mean1",mean1,'std1',std1)
    print("mean2",mean2,'std2',std2)
        
    #Generate granular rectangle dataloder
   # GRdataloader = DataLoader(GRdataset,batch_size=args.batch_size,num_workers=args.num_workers,shuffle=True)
   # GRdataloader_eval = DataLoader(GRval_dataset,batch_size=args.batch_size)
    ...
    num_features = GRdataset.get_dim_node_features()  # Get node feature dimensions
    GBmodel = GCN_8_plus(num_features=num_features, initdim=256, inithead=8).cuda()  
    
    for m in model.modules():
        if isinstance(m, nn.BatchNorm2d):
            m.momentum = 0.01

    runner = GHBR(model=model,
                 GBmodel=GBmodel,
   ...
   # optimizer = torch.optim.AdamW(runner.model.cnn_graph_attentionfusion.parameters(), lr=args.lr_encoder, weight_decay=args.weight_decay)
    scheduler = get_multistep_schedule_with_warmup(optimizer, milestones=[1e10], gamma=0.2,
                                                   num_warmup_steps=0)
    runner.set_optimizer(optimizer, scheduler)

    #Set optimizer for granular rectangle
    GBoptimizer = torch.optim.SGD(GBmodel.parameters(), lr=args.lr, weight_decay=args.weight_decay)
    
    GBscheduler = get_multistep_schedule_with_warmup(GBoptimizer,milestones=[1e10],gamma=0.2,
                                                   num_warmup_steps=0)
    runner.set_GBoptimizer(GBoptimizer,GBscheduler)
...
if __name__ == "__main__":
    ...
    '''
    parser.add_argument('--save_dir', type=str, default='./saved_models') #保存模型的训练结果
    parser.add_argument('-sn', '--save_name', type=str,
                        default='ghbr_br35h_256')
    ...
    parser.add_argument('--lr_encoder', type=float, default=6e-5)
    ...
    Data Configurations
    '''
    parser.add_argument('--data_dir', type=str, default=r"E:/Br35H") 
    parser.add_argument('--data_path',type=str,default=r"E:\OURS-update\br35h_h5\train") 
    parser.add_argument('--eval_data_path',type=str,default=r"E:\OURS-update\br35h_h5\val") 
